...
def text_to_audio(text):
    try:
        engine = pyttsx3.init()
        audio_path = f"audio_{int(datetime.now().timestamp())}.mp3"
        engine.save_to_file(text, audio_path)
        engine.runAndWait()
        return audio_path
    except Exception as e:
        st.error(f"❌ Offline voice generation failed: {e}")
        return None
...
"""
        ...
        if voice_toggle:
            ...
            if audio_path and os.path.exists(audio_path):
                with open(audio_path, "rb") as f:
                    ...
                    st.info("🔊 Voice playback generated.")
        # 🔁 Don't delete here to avoid PermissionError
        # You can clean up files manually or during a future session

        else:
            st.error("⚠️ Voice playback failed.")
    ...
    # ====== CLEANUP ======
    st.markdown("### 🧹 Cleanup")
    if st.button("🧹 Clean Up Old Audio Files"):
        for f in os.listdir():
            if f.startswith("audio_") and f.endswith(".mp3"):
                try:
                    os.remove(f)
                except Exception as e:
                    st.warning(f"Couldn't remove {f}: {e}")
    ...
    if role == "admin":
        ...
        if len(all_hist) > 0:
            ...
            st.markdown("### 🔄 Usage Control")
    if st.button("♻️ Reset All Users' Usage Counters"):
        df = pd.read_csv(USERS_CSV)
        df["used_today"] = 0
        df["last_used_date"] = ""
        df.to_csv(USERS_CSV, index=False)
        st.success("✅ All usage counters have been reset.")
